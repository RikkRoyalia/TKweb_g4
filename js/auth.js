  ...
  if (activeUser) {
    authArea.innerHTML = `
      <div class="flex flex-col items-center justify-start">
        <div class="flex flex-col items-center text-[13px] text-[#1f2937] hover:text-[#2563eb] cursor-pointer">
          ...
          <span class="font-medium whitespace-nowrap hidden sm:inline">${activeUser}</span>
        </div>
        <button onclick="window.xuLyDangXuat()"
                class="mt-[6px] px-[14px] py-[4px] bg-[#fee2e2] text-[#ef4444] rounded-full text-[11px] font-bold hover:bg-[#ef4444] hover:text-white transition-all shadow-sm whitespace-nowrap hidden sm:block">
          Đăng xuất
        </button>
      </div>
    `;
    ...
    authArea.innerHTML = `
      <a href="login.html"
        ...
        <span class="text-[22px] mb-[4px]"><i class="fa-regular fa-user"></i></span>
        <span class="font-medium whitespace-nowrap hidden sm:inline">Đăng nhập</span>
      </a>
    `;
  }
});
